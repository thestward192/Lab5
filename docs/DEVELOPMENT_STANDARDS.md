...
- Orden de imports/requires
  ...
  - Nota: el lint aplica a archivos .js del repositorio; scripts inline dentro de HTML no se lintéan por defecto.
...
- BDD (Behavior-Driven Development) (obligatorio)
  ...
  - Los escenarios deben cubrir el “happy path” y al menos 1 caso de borde relevante por funcionalidad.
  - Comandos:
...
- Runtime
  - Express: ^4.18.2
  - Socket.IO (servidor): ^4.7.2
- Desarrollo
  - Mocha (unit tests): ^10.2.0
  - Cucumber.js (BDD): ^10.0.0
  - ESLint (linting): ^8.57.0
- Frontend (CDN en `index.html`)
  - Socket.IO client: 4.7.2 (CDN)
  - jQuery: 1.11.1 (uso auxiliar; puede sustituirse o retirarse en futuras mejoras)

Notas

- El lint aplica a archivos .js del repositorio (no a scripts inline del HTML por defecto).
- Las versiones se fijan para estabilidad; pueden actualizarse con pruebas y revisión de breaking changes.
